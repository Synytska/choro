...
as $$
  with streak_dates as (
    select distinct coalesce(child_tasks.due_at::date, child_tasks.created_at::date) as completed_date
    from public.child_tasks
    where child_tasks.child_id = input_child_id
      and child_tasks.status in ('review', 'done')
  ),
  streak_groups as (
    select
      streak_dates.completed_date,
      streak_dates.completed_date
        - (row_number() over (order by streak_dates.completed_date))::integer as streak_group
    from streak_dates
  ),
  longest_streak as (
